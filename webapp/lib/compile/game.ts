...
export class Game {
  ...
  private doAction(a: Action): void {
    ...
    const ap = st.currentPlayer;
    if (a.type === "REFRESH") {
      // `refreshPlayer` is now a generator (yields the control-rearrange
      // prompt when `ap` holds the control component). Push it onto
      // _pending; the engine drains it immediately after this action
      // returns.
      this.pushEffect(refreshPlayer(st, ap));
